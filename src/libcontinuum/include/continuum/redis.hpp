#ifndef CONTINUUM_LIB_REDIS_HPP
#define CONTINUUM_LIB_REDIS_HPP
...
/**
 * Elements of this vector should not appear as substrings of any input Continuum
...
bool send_cmd_no_reply(redox::Redox& redis,
  ...
  if (!cmd.ok()) {
    ...
    log_debug_formatted(LOGGING_TAG_REDIS, "Successfully issued command \"{}\"",
 ...
 * \param model_data_path should be the path on the Continuum host
 ...
 * the rest of the Continuum codebase.
 *
...
/**
 * Looks up backend names listed in the backend table.
 *
 * \return Returns a vector of backend names as strings. If no backend names
 * were found, an empty vector will be returned.
 */
...
/**
 * Adds retrain data into the data table.
 *
 * \return Returns pair : true of the add was successful, the key of data in
 * redis.
 */
...
/**
 * Adds link between retrain data and application name.
 *
 * \return Returns true of the add was successful.
 */
...
/**
 * Gets ids of data which arrive between the begin timestamp and the end one.
 *
 * \return Returns a list pf data ids.
 */
...
/**
 * Adds a backend into the backend table. This will
 * overwrite any existing entry with the same key.
 *
 * \return Returns true of the add was successful.
 */
bool add_backend(redox::Redox& redis, const std::string& backend_name,
...
/**
 * Link a backend to an app. This will overwrite the existing
 * backend link.
 *
 * @return Returns true of the add was successful.
 */
...
/**
 * Looks up backend link for the given app.
 *
 * @return Return empty string on failure and backend name on
 *         success.
 */
...
/**
 * Looks up a backend based on its name.
 *
 * \return Returns a map of backend attribute name-value pairs as
 * strings. If the backend was not found, an empty map will be
 * returned.
 */
...
/**
 * Subscribes to changes in the app backend link table. The
 * callback is called with the string key of the application
 * that changed and the Redis event type. The key can
 * be used to look up the new value. The message type identifies
 * what type of change was detected. This allows subscribers
