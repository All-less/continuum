#ifndef CONTINUUM_LIB_DATATYPES_H
#define CONTINUUM_LIB_DATATYPES_H
...
using RetrainAck = bool;

enum class RetrainType {
  StartRetrain = 0,
};
...
// TODO add more detailed error cases
enum class RetrainResult {
  Started = 0,
  ErrorStart = 1,
  Finished = 2,
  ErrorFinish = 3,
...
class RetrainRequest {
 public:
  ~RetrainRequest() = default;
  RetrainRequest(long data_from, long data_to,
               std::vector<std::string> batch_ids, long data_size,
               RetrainType type);

  RetrainRequest(const RetrainRequest &) = default;
  RetrainRequest &operator=(const RetrainRequest &) = default;

  RetrainRequest(RetrainRequest &&) = default;
  RetrainRequest &operator=(RetrainRequest &&) = default;

  /**
   * Serialize batch_ids_ with the same procedure as in PredictRequest::serialize().
   */
...
namespace std {
template <>
struct hash<continuum::VersionedModelId> {
  typedef std::size_t result_type;
  std::size_t operator()(const continuum::VersionedModelId &vm) const {
    std::size_t seed = 0;
...
#endif  // CONTINUUM_LIB_DATATYPES_H
