...
double try_get_double(rapidjson::Value& d, const char* key_name,
  ...
  if (!d.IsObject() || !d.HasMember(key_name)) {
...
std::shared_ptr<Input> parse_single_input(InputType input_type,
  ...
  switch (input_type) {
    case InputType::Doubles: {
      ...
      return std::make_shared<continuum::DoubleVector>(inputs);
    }
    case InputType::Floats: {
      std::vector<float> inputs = get_float_array(d, "input");
      return std::make_shared<continuum::FloatVector>(inputs);
    }
    case InputType::Ints: {
      std::vector<int> inputs = get_int_array(d, "input");
      return std::make_shared<continuum::IntVector>(inputs);
    }
    case InputType::Strings: {
      std::string input_string = get_string(d, "input");
      return std::make_shared<continuum::SerializableString>(input_string);
    }
    case InputType::Bytes: {
      std::vector<uint8_t> inputs = get_base64_encoded_byte_array(d, "input");
      return std::make_shared<continuum::ByteVector>(inputs);
    }
  ...
  switch (input_type) {
    case InputType::Doubles: {
      ...
      for (auto input : input_batch) {
        result.push_back(std::make_shared<continuum::DoubleVector>(input));
      }
      return result;
    }
    case InputType::Floats: {
      ...
      std::vector<std::shared_ptr<Input>> result;
      for (auto input : input_batch) {
        result.push_back(std::make_shared<continuum::FloatVector>(input));
      }
      return result;
    }
    case InputType::Ints: {
      ...
      std::vector<std::shared_ptr<Input>> result;
      for (auto input : input_batch) {
        result.push_back(std::make_shared<continuum::IntVector>(input));
      }
      return result;
    }
    case InputType::Strings: {
      ...
      std::vector<std::shared_ptr<Input>> result;
      for (auto input : input_batch) {
        result.push_back(std::make_shared<continuum::SerializableString>(input));
      }
      return result;
    }
    case InputType::Bytes: {
      ...
      std::vector<std::shared_ptr<Input>> result;
      for (auto input : input_batch) {
        result.push_back(std::make_shared<continuum::ByteVector>(input));
      }
      return result;
    }
    default: throw std::invalid_argument("input_type is not a valid type");
...
void add_backend_version_from_redis(
    rapidjson::Document& d,
    const std::unordered_map<std::string, std::string>& backend_metadata) {
  std::string key = "backend_version";
  check_key_exists_in_map(key, backend_metadata);
  add_string(d, key.c_str(), backend_metadata.at(key));
}
...
void redis_backend_metadata_to_json(
    ...
    const std::unordered_map<std::string, std::string>& backend_metadata) {
  d.SetObject();
  add_backend_version_from_redis(d, backend_metadata);
}
...
void add_model_labels_from_redis(
  ...
  std::vector<std::string> labels_vec =
      continuum::redis::str_to_labels(model_metadata.at(key));
