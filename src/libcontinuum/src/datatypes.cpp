...
size_t FloatVector::hash() const {
  // TODO: Find an alternative to hashing floats directly, as this
...
size_t DoubleVector::hash() const {
  // TODO: Find an alternative to hashing doubles directly, as
...
void rpc::PredictionRequest::validate_input_type(
  ...
  if (input->type() != input_type_) {
    ...
    log_error(LOGGING_TAG_CONTINUUM, ss.str());
...
RetrainRequest::RetrainRequest(long data_from, long data_to,
                               std::vector<std::string> data_ids,
                               long data_size, RetrainType type)
    : data_from_(data_from),
      data_to_(data_to),
      batch_ids_(data_ids),
      data_size_(data_size),
      type_(type) {}

std::vector<ByteBuffer> RetrainRequest::serialize() {
  if (batch_ids_.size() <= 0) {
    throw std::length_error(
        "Attempted to serialize a request with no input data!");
  }

  std::vector<uint32_t> request_metadata;
  request_metadata.emplace_back(static_cast<uint32_t>(type_));

  ByteBuffer serialized_ids;
  for (int i = 0; i < (int)batch_ids_.size(); i++) {
    ByteBuffer tmp(batch_ids_[i].begin(), batch_ids_[i].end());
    serialized_ids.insert(serialized_ids.end(), tmp.begin(), tmp.end());
    serialized_ids.push_back('\0');
  }
  ...
  serialized_request.push_back(serialized_request_metadata);
  serialized_request.push_back(serialized_ids);
