...
TEST_F(RedisTest, AddModel) {
  ...
  std::string container_name = "continuum/test_container";
...
TEST_F(RedisTest, AddRetrainData) {
  std::vector<double> data1{1.0, 0.0, 0.5};
  std::vector<double> data2{2.0, 1.0, 1.5};
  std::vector<std::vector<double>> data_list{data1, data2};

  auto result = add_retrain_data(*redis_, 1, data_list);
  ASSERT_TRUE(result.first);
}

TEST_F(RedisTest, AddAppDataLink) {
  std::vector<double> data1{1.0, 0.0, 0.5};
  std::vector<double> data2{2.0, 1.0, 1.5};
  std::vector<std::vector<double>> data_list{data1, data2};
  ...
  ASSERT_TRUE(add_application(*redis_, name, input_type, policy, default_output,
                              latency_slo_micros));
  ASSERT_TRUE(add_app_data_link(*redis_, name, 1, result.second));
}

TEST_F(RedisTest, GetRetrainDataIds) {
  std::vector<double> data1{1.0, 0.0, 0.5};
  std::vector<double> data2{2.0, 1.0, 1.5};
  std::vector<std::vector<double>> data_list1{data1, data2};
  std::vector<std::vector<double>> data_list2{data2, data1};

  auto result1 = add_retrain_data(*redis_, 10, data_list1);
  ASSERT_TRUE(result1.first);

  auto result2 = add_retrain_data(*redis_, 20, data_list1);
  ...
  ASSERT_TRUE(add_application(*redis_, name, input_type, policy, default_output,
                              latency_slo_micros));
  ASSERT_TRUE(add_app_data_link(*redis_, name, 10, result1.second));
  ASSERT_TRUE(add_app_data_link(*redis_, name, 20, result2.second));

  auto final_result = get_retrain_data_ids(*redis_, name, 5, 15);
  EXPECT_EQ(final_result.size(), static_cast<size_t>(1));

  EXPECT_EQ(final_result[0], result1.second);
}

TEST_F(RedisTest, GetAllBackendNames) {
  std::vector<std::string> backend_names{"backend1", "backend2", "backend3"};
  for (auto name : backend_names) {
    ASSERT_TRUE(add_backend(*redis_, name, "1.0", "", 0.0, 0.0, 0.0, 0));
  }
  auto res_names = get_all_backend_names(*redis_);
  for (auto expected_name : backend_names) {
    auto res = std::find(res_names.begin(), res_names.end(), expected_name);
    auto found = (res != res_names.end());
    ASSERT_TRUE(found);
  }
}
...
TEST_F(RedisTest, AddBackend) {
  std::string name = "my_backend_name";
  std::string version = "1.0";
  int zmq_connection_id = 0;
  std::string policy = "ManualPolicy";
  double alpha = 0.0;
  double beta = 0.0;
  double weight = 0.0;
  ASSERT_TRUE(add_backend(*redis_, name, version, policy, alpha,
                          beta, weight, zmq_connection_id));
  auto result = get_backend(*redis_, name);
  EXPECT_EQ(result.size(), static_cast<size_t>(6));
  EXPECT_EQ(result["backend_version"], version);
  EXPECT_EQ(std::stoi(result["zmq_connection_id"]), zmq_connection_id);
  EXPECT_EQ(result["policy"], policy);
  EXPECT_EQ(std::stod(result["alpha"]), alpha);
  EXPECT_EQ(std::stod(result["beta"]), beta);
  EXPECT_EQ(std::stod(result["weight"]), weight);
}

TEST_F(RedisTest, SetBackendLink) {
  std::string app_name = "my_app_name";
  std::string backend_name = "my_backend_name";
  ASSERT_TRUE(set_backend_link(*redis_, app_name, backend_name));
  auto result = get_backend_link(*redis_, app_name);
  EXPECT_EQ(result, backend_name);
}
...
TEST_F(RedisTest, SubscriptionDetectBackendLinkAdd) {
  std::string app_name = "myapp";
  std::string backend_name = "mybackend";
  std::condition_variable_any notif_recv;
  std::atomic<bool> recv{false};
  std::mutex notif_mutex;

  subscribe_to_backend_link_changes(
      *subscriber_,
      [&notif_recv, &notif_mutex, &recv, app_name] (
          const std::string& key, const std::string& event_type) {
        ASSERT_EQ(event_type, "set");
        std::unique_lock<std::mutex> l(notif_mutex);
        recv = true;
        ASSERT_EQ(key, app_name);
        notif_recv.notify_all();
      });
  std::this_thread::sleep_for(std::chrono::milliseconds(500));
  ...
  std::unique_lock<std::mutex> l(notif_mutex);
  bool result = notif_recv.wait_for(l, std::chrono::milliseconds(1000),
                                    [&recv]() { return recv == true; });
  ASSERT_TRUE(result);
}
