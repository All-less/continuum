...
TEST(InputSerializationTests, EmptySerialization) {
  continuum::rpc::PredictionRequest request(InputType::Bytes);
  try {
    request.serialize();
...
TEST(InputSerializationTests, ByteSerialization) {
  ...
  std::vector<continuum::ByteBuffer> serialized_request = request.serialize();
  ASSERT_EQ(serialized_request.size(), SERIALIZED_REQUEST_SIZE);
  continuum::ByteBuffer request_header = serialized_request[0];
  continuum::ByteBuffer input_header_size = serialized_request[1];
  continuum::ByteBuffer input_header = serialized_request[2];
  continuum::ByteBuffer input_content_size = serialized_request[3];
  continuum::ByteBuffer input_content = serialized_request[4];
  ...
  ASSERT_EQ(*raw_request_type,
            static_cast<uint32_t>(continuum::RequestType::PredictRequest));
  uint32_t* typed_input_header =
      reinterpret_cast<uint32_t*>(input_header.data());
  ASSERT_EQ(*typed_input_header,
            static_cast<uint32_t>(continuum::InputType::Bytes));
  typed_input_header++;
...
TEST(InputSerializationTests, IntSerialization) {
  continuum::rpc::PredictionRequest request(InputType::Ints);
  ...
  uint32_t* typed_input_header =
      reinterpret_cast<uint32_t*>(input_header.data());
  ASSERT_EQ(*typed_input_header,
            static_cast<uint32_t>(continuum::InputType::Ints));
  typed_input_header++;
  uint32_t num_inputs = *typed_input_header;
  ASSERT_EQ(num_inputs, static_cast<uint32_t>(data_vectors.size()));
  typed_input_header++;
...
TEST(InputSerializationTests, FloatSerialization) {
  continuum::rpc::PredictionRequest request(InputType::Floats);
  ...
  uint32_t* typed_input_header =
      reinterpret_cast<uint32_t*>(input_header.data());
  ASSERT_EQ(*typed_input_header,
            static_cast<uint32_t>(continuum::InputType::Floats));
  typed_input_header++;
  uint32_t num_inputs = *typed_input_header;
  ASSERT_EQ(num_inputs, static_cast<uint32_t>(data_vectors.size()));
  typed_input_header++;
...
TEST(InputSerializationTests, DoubleSerialization) {
  continuum::rpc::PredictionRequest request(InputType::Doubles);
  ...
  uint32_t* typed_input_header =
      reinterpret_cast<uint32_t*>(input_header.data());
  ASSERT_EQ(*typed_input_header,
            static_cast<uint32_t>(continuum::InputType::Doubles));
  typed_input_header++;
  uint32_t num_inputs = *typed_input_header;
  ASSERT_EQ(num_inputs, static_cast<uint32_t>(data_vectors.size()));
  typed_input_header++;
...
TEST(InputSerializationTests, StringSerialization) {
  continuum::rpc::PredictionRequest request(InputType::Strings);
  ...
  uint32_t* typed_input_header =
      reinterpret_cast<uint32_t*>(input_header.data());
  ASSERT_EQ(*typed_input_header,
            static_cast<uint32_t>(continuum::InputType::Strings));
  typed_input_header++;
  uint32_t num_inputs = *typed_input_header;
